...
class BuildError(models.Model):
    ...
    first_seen_date = fields.Datetime(string='First Seen Date', compute='_compute_first_seen_date')
    last_seen_build_id = fields.Many2one('runbot.build', compute='_compute_last_seen_build_id', string='Last Seen build', store=True)
    last_seen_date = fields.Datetime(string='Last Seen Date', compute='_compute_last_seen_date', store=True)
    ...
    def _compute_last_seen_date(self):
        for build_error in self:
            first_build_error_link_id = (build_error.build_error_link_ids | build_error.child_ids.build_error_link_ids)[0] or False
            build_error.last_seen_date = first_build_error_link_id and first_build_error_link_id.log_date
    ...
    @api.depends('build_error_link_ids', 'child_ids')
    def _compute_first_seen_date(self):
        for build_error in self:
            first_build_error_link_id = (build_error.build_error_link_ids | build_error.child_ids.build_error_link_ids)[-1] or False
            build_error.first_seen_date = first_build_error_link_id and first_build_error_link_id.log_date
